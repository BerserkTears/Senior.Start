<?php 
    require 'db.php';
    $data=$_POST;
    if(isset($data['naznachit'])){
        $sob=R::dispense('sobesedovaniya');
        $sob->company = $_SESSION['logged_user']->company;
        $sob->companyinn = $_SESSION['logged_user']->inn;
        $sob->user = $data['user_sob'];
        $sob->date = $data['sob_date'];
        R::store($sob);
    }
?>
<!DOCTYPE html>
